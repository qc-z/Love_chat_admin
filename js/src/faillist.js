...
window.onload = function(){
	$.ajax({
            type: "GET",
            url: server + "/getLists?auditStatus=failed",
            dataType: 'json',
            xhrFields: {
...
function addList(data){
	...
	var list = 	data.length	
	    ...
	    for(var k = 0;k < data[i].photoPub.length;k++){
		    ...
		    str += '<tr><th colspan="2"><span class="listNumber pull-left">'+(i+1)+'</span><span class="failing">审核不通过</span><span>(审核时间：'+moment(data[i].auditAt).format('YYYY-MM-DD HH:mm:ss')+')</span><span class="listbutton pass pull-right" data-id='+data[i]._id+'>审核通过</span><span '+data[i]._id+'data-prive='+priArr+' '+data[i]._id+'data-public='+pubArr+' failid='+data[i]._id+' class="'+data[i]._id+' listbutton faile pull-right reject">再次拒绝</span></th></tr>'
...
function addOneList(data){
	...
	var str = ''
		str += '<div classs="content">'
	    str += '<table cellspacing="0">'
		    str += '<tr><th colspan="2"><span class="listNumber pull-left">1</span><span class="failing">审核不通过</span><span class="listbutton pass pull-right" data-id='+data._id+'>审核通过</span><span>(审核时间：'+moment(data.auditAt).format('YYYY-MM-DD HH:mm:ss')+')</span><span '+data._id+'data-prive='+priArr+' '+data._id+'data-public='+pubArr+' failid='+data._id+' class="'+data._id+' listbutton faile pull-right reject">再次拒绝</span></th></tr>'
...
$("#searchbtn").click(function(){
	...
	if(!isNaN(searchName) && searchName.length == 11){ 
		data = {mobile:searchName,auditStatus:'failed'}
    }else if(isNaN(searchName)){
		data = {nickName:searchName,auditStatus:'failed'}
...
function prenext(pagination){
	$.ajax({
            type: "GET",
            url: server + "/findByNumber?pagination="+pagination+"&auditStatus=failed",
            dataType: 'json',
            xhrFields: {
                withCredentials: true
            },
