...
(function(window, document, $, undefined){
  ...
  $(function(){
    ...
    $('#addSensitiveBtn').click(function(e) {
      ...
      var regNu = new RegExp("^[0-9]*$");
      if(!reg.test(content)){
        layer.msg("请输入数字和小写字母!", {time: 1000});
          return
      }
      if(content.length != 5){
          layer.msg("长度必须是五位!", {time: 1000});
          return
      }
      if(regEn.test(content) || regNu.test(content)){
          layer.msg("必须是字母加数字", {time: 1000});
          return
      }
 
      var methods = $("#select").find("option:selected").val()
      ...
      httpAjax.post('/addCoupon', body, function(results) {
      	$("#addModal").modal('hide')
